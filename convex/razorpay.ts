...
export const validateRazorPayRequest = async (
  signature: string,
  body: string
) => {
  const razorPayWebhookSecret = ensureEnvironmentVariable(
    "RAZORPAY_WEBHOOK_SECREET"
  );
...
export const handleRazorPayWebhook = internalAction({
  ...
  handler: async (ctx, { signature, body }) => {
    try {
      ...
      if (!isValid) {
        console.log(`Invalid razorpay request, valdiation failed`);
        return { success: false };
      }

      const requestBody = JSON.parse(body) as unknown as Event;
      const { event, payload } = requestBody;
      const { id, notes, amount, created_at, method, status, currency } =
        payload.payment.entity;
      console.log(
        `payment webhook from razorpay: ${id} | ${notes.email} | ${notes.phone} | ${event}`
      );
      switch (event) {
