...
export const prepareBatch1 = action({
  ...
  handler: async (ctx, { planId }) => {
    ...
    try {
      ...
      const modelName = await generatebatch1(emptyPlan.userPrompt);
      console.log("done");
...
export const prepareBatch2 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const emptyPlan = await fetchEmptyPlan(ctx, planId);
      if (!emptyPlan) throw new ConvexError("Empty plan not found");

      const planMetadata = await ctx.runQuery(
        internal.planSettings.getPlanSettings,
        { planId: emptyPlan._id }
      );
      if (!planMetadata) throw new ConvexError("Plan metadata not found");

      const { activityPreferences, companion, fromDate, toDate } = planMetadata;

      const result = await generatebatch2({
        userPrompt: emptyPlan.userPrompt,
        activityPreferences,
        companion,
        fromDate,
      ...
      if (
        ...
        throw new ConvexError("Incomplete Gemini response: Batch2 fields missing");
      }
...
export const prepareBatch3 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const result = await generatebatch3({
        userPrompt: emptyPlan.userPrompt,
        activityPreferences,
        companion,
        fromDate,
      ...
      const { itinerary, topplacestovisit } = result;

      if (!itinerary || !topplacestovisit) {
        throw new ConvexError("Incomplete Gemini response: Batch3 fields missing");
      }
